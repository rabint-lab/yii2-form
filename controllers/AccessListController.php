...
use Yii;
use rabint\form\models\AccessList;
use rabint\form\models\search\AccessListSearch;
...
/**
 * AccessListController implements the CRUD actions for AccessList model.
 */
class AccessListController extends \rabint\controllers\AdminController {
    ...
    public function actionBulk($action)
    {
        ...
        switch ($action) {
            case static::BULK_ACTION_SETPUBLISH:
                if (AccessList::updateAll(['status' => AccessList::STATUS_DRAFT], ['id' => $selection])) {
            ...
            case static::BULK_ACTION_DELETE:
                if (AccessList::deleteAll(['id' => $selection])) {
                    Yii::$app->session->setFlash('success',  Yii::t('rabint', 'Bulk action was successful'));
                } else {
                    $err++;
                }
    ...
    /**
     * Displays a single AccessList model.
     * @param integer $id
     * @return mixed
     */
    ...
    /**
     * Creates a new AccessList model.
            ...
            if ($model->save()) {
                ...
                if ($request->isAjax) {
                    ...
                    return [
                        ...
                        'title' =>  Yii::t('rabint', 'Create new').' '. Yii::t('rabint', 'AccessList'),
    ...
    /**
     * Updates an existing AccessList model.
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->save()) {
                ...
                if ($request->isAjax) {
                    ...
                    [
                        'forceReload' => '#ajaxCrudDatatable',
                        'title' =>  Yii::t('rabint', 'Updating').' '. Yii::t('rabint', 'AccessList'),
    ...
    /**
     * Deletes an existing AccessList model.
    ...
    /**
     * Finds the AccessList model based on its primary key value.
     * If the model is not found, a 404 HTTP exception will be thrown.
     * @param integer $id
     * @return AccessList the loaded model
     * @throws NotFoundHttpException if the model cannot be found
     */
    protected function findModel($id)
    {
        if (($model = AccessList::findOne($id)) !== null) {
            return $model;
        } else {
