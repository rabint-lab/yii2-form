...
use rabint\form\models\search\FormSearch;
 ...
 */
class FormController extends \rabint\controllers\AdminController {
    ...
    public function actionBulk($action)
    {
        ...
        switch ($action) {
            case static::BULK_ACTION_SETPUBLISH:
                if (Form::updateAll(['status' => Form::STATUS_DRAFT], ['id' => $selection])) {
            ...
            case static::BULK_ACTION_DELETE:
                if (Form::deleteAll(['id' => $selection])) {
                    Yii::$app->session->setFlash('success',  Yii::t('rabint', 'Bulk action was successful'));
                } else {
                    $err++;
                }
            ...
            if ($model->save()) {
                ...
                if ($request->isAjax) {
                    ...
                    return [
                        ...
                        'title' =>  Yii::t('rabint', 'Create new').' '. Yii::t('rabint', 'Form'),
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->save()) {
                ...
                if ($request->isAjax) {
                    ...
                    [
                        'forceReload' => '#ajaxCrudDatatable',
                        'title' =>  Yii::t('rabint', 'Updating').' '. Yii::t('rabint', 'Form'),
