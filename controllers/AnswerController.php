...
use rabint\form\models\search\AnswerSearch;
 ...
 */
class AnswerController extends \rabint\controllers\AdminController {
    ...
    public function actionBulk($action)
    {
        ...
        switch ($action) {
            case static::BULK_ACTION_SETPUBLISH:
                if (Answer::updateAll(['status' => Answer::STATUS_DRAFT], ['id' => $selection])) {
            ...
            case static::BULK_ACTION_DELETE:
                if (Answer::deleteAll(['id' => $selection])) {
                    Yii::$app->session->setFlash('success',  Yii::t('rabint', 'Bulk action was successful'));
                } else {
                    $err++;
                }
            ...
            if ($model->save()) {
                ...
                if ($request->isAjax) {
                    ...
                    return [
                        ...
                        'title' =>  Yii::t('rabint', 'Create new').' '. Yii::t('rabint', 'Answer'),
        ...
        if ($model->load(Yii::$app->request->post())) {
            if ($model->save()) {
                ...
                if ($request->isAjax) {
                    ...
                    [
                        'forceReload' => '#ajaxCrudDatatable',
                        'title' =>  Yii::t('rabint', 'Updating').' '. Yii::t('rabint', 'Answer'),
